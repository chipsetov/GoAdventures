pipeline {
  ...
  stages {
    stage('test') {
      ...
      steps {
        script {
          browser = sh(returnStdout: true, script: 'ssh 35.246.214.201 -oStrictHostKeyChecking=no -c "diff /etc/bind/db.goadventures.com /etc/bind/db.goadventures.com-green; true"')
        }
