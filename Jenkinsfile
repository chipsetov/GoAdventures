pipeline {
  ...
  stages {
    stage('test') {
      ...
      steps {
        script {
          browser = sh(returnStdout: true, script: 'ssh shakh_anton@35.246.214.201 -oStrictHostKeyChecking=no diff /etc/bind/db.goadventures.com /etc/bind/db.goadventures.com-blue')
        }
